...
class ChatScreen extends React.Component {
	...
	getCallChannelsListner = (user_id) => {
		...
		this.callchannel_listener = call_channel_collection
			...
			<View style={styles.searchContainer}>
				...
				<Menu>
					...
					<MenuOptions optionsContainerStyle={styles.popupContainer}>
						<MenuOption
							onSelect={() => {
								this.props.navigation.push(RouteNames.InvitationsScreen);
							}}
						>
							<View
								style={{
							...
							>
								...
								{this.props.new_invites.length > 0 && (
									...
									>
										({this.props.new_invites.length} new)
									</Text>
								)}
							</View>
						</MenuOption>
						...
						<MenuOption
							onSelect={() => {
								this.props.navigation.push(RouteNames.StorySettingScreen);
							}}
						>
							<View
								style={{
	...
	renderCallItem(item, index) {
		...
		return (
			<TouchableOpacity
				style={[Theme.styles.row_center, styles.chatContainer]}
				onPress={() => {
				...
				<View style={{ flex: 1 }}>
					...
					<View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
						<View>
							...
							{!isEmpty(duration) && <Text style={styles.duration}>{duration}</Text>}
						</View>
		...
		return (
			<FlatList
				...
				renderItem={(item, index) =>
					this.state.isChatList ? this.renderChatItem(item) : this.renderCallItem(item)
				}
				...
				ListEmptyComponent={() =>
					this.state.isChatList && this.state.isLoadingChat == false ? (
						...
						!this.state.isChatList && this.state.isLoadingCall == false && <NoCalls />
					)
				}
				ListHeaderComponent={
	...
	renderSnapFoodMapButton() {
		return (
			<View style={styles.SnapFoodMapContainer}>
				<TouchableOpacity
					style={[Theme.styles.col_center, styles.SnapFoodMapButton]}
					onPress={() => {
						this.props.navigation.push(RouteNames.SnapfoodMapScreen);
					}}
				>
					...
					<FastImage
						source={require('../../../common/assets/images/chat/open_maps.png')}
			...
			if (item.channel_type == 'single' || item.channel_type == 'admin_support') {
				if (item.last_msg.map != null) {
					return isMe
						...
						: translate('social.chat.user_shared_location');
				} else if (item.last_msg.emoji != null) {
					return item.last_msg.emoji != null && item.last_msg.emoji.length > 0
						? item.last_msg.emoji.map((item) => item.code).join('')
						: '';
					...
					return isMe
						? translate('social.chat.you_shared_photo')
						: translate('social.chat.user_shared_photo');
				} else if (item.last_msg.audio != null) {
					return isMe
						? translate('social.chat.you_shared_audio')
						: translate('social.chat.user_shared_audio');
				...
				if (item.last_msg.map != null) {
					...
					return (
						last_msg_user +
						': ' +
						(isMe ? translate('social.chat.you_sent_emoji') : translate('social.chat.user_sent_emoji'))
					);
					...
					return (
						last_msg_user +
						': ' +
						(isMe ? translate('social.chat.you_shared_photo') : translate('social.chat.user_shared_photo'))
					);
				} else if (item.last_msg.audio != null) {
					return (
						last_msg_user +
						': ' +
						(isMe ? translate('social.chat.you_shared_audio') : translate('social.chat.user_shared_audio'))
					);
				} else if (item.last_msg.text != null) {
	...
	render() {
		return (
			<View style={styles.container}>
				...
				{this.renderHistory()}
				<NewConvOptionModal
					showModal={this.state.showConvModal}
					goChat={() => {
				...
				<ConfirmModal
					showModal={this.state.isDeleteConfirm}
					title={
					...
					yes={translate('social.delete_confirm_yes')}
					no={translate('social.delete_confirm_no')}
