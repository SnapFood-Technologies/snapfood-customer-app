...
class SnapfooderScreen extends React.Component {
	...
	getSnapfooderGallery = async (user_id) => {
		apiFactory
			.get(`users/gallery?user_id=${user_id}`)
			.then(({ data }) => {
	...
	onRemoveFriend = async () => {
		...
		let message =
			this.props.user.sex == 'female'
				? translate('social.delete_friend_female_confirm')
				: translate('social.delete_friend_confirm');
		message = message.replace('{user}', this.state.user.username || this.state.user.full_name);
		alerts.confirmation(translate('alerts.confirmation'), message).then(async () => {
			await this.setState({ btnLoading: true });
			apiFactory
				.post(`users/friends/remove`, {
					user_id: this.props.user.id,
	...
	renderSuggestedUsers = (items) => {
		...
		return (
			<View style={[Theme.styles.col_center_start, styles.userHorizList]}>
	...
	renderMyContactsItem = () => {
		return (
			<View style={[Theme.styles.col_center, { justifyContent: 'flex-start', marginRight: 15 }]}>
				<TouchableOpacity
					activeOpacity={0.9}
					style={[Theme.styles.col_center, styles.mycontactsView]}
	...
	renderBioBlock = () => {
		...
		if (
			!isEmpty(this.state.user.bio_text) &&
			(this.state.isMe ||
				this.state.user.bio_text_public == 1 ||
				(this.state.isFriend && this.state.user.bio_text_public == 0))
		) {
			isBioVisible = true;
		}
				...
				<View style={[{ width: '100%', paddingHorizontal: 20 }]}>
					<View style={[styles.bioBlock]}>
						{this.state.galleryImages.length > 0 && (
							<TouchableOpacity
								style={[Theme.styles.row_center, styles.bioPhotoBtn]}
								onPress={() => {
									this.setState({
										galleryVisible: true,
									});
						...
						{isBioVisible && this.state.galleryImages.length > 0 && <View style={styles.bioblockDivider} />}
						{!isEmpty(this.state.user.bio_text) && (
							<AppText style={styles.bioTxt}>{this.state.user.bio_text}</AppText>
						)}
						...
						{this.state.interests.filter((i) => i.category == 'social').length > 0 && (
							<>
								<AppText style={styles.interestCatTxt}>
									{translate('interests.social_interest')}
								</AppText>
								<AppText style={styles.interestTxt}>
									{this.state.interests
										.filter((i) => i.category == 'social')
										.map((i) => (this.props.language == 'en' ? i.title_en : i.title))
										.join(', ')}
								</AppText>
							</>
						)}
						{this.state.interests.filter((i) => i.category == 'food').length > 0 && (
							<>
								<AppText
									style={[
										styles.interestCatTxt,
										this.state.interests.filter((i) => i.category == 'social').length > 0 && {
											marginTop: 8,
										},
									]}
								>
									{translate('interests.food_interest')}
								</AppText>
								<AppText style={styles.interestTxt}>
									{this.state.interests
		...
		return (
			<View style={styles.container}>
				...
				<ScrollView style={styles.container}>
					...
					{this.renderBioBlock()}
					{this.renderMap()}
					{this.state.suggestedLoading == false &&
					...
					<View style={[{ width: '100%', paddingHorizontal: 20, marginBottom: 20 }]}>
						...
						{isFriend ? (
							...
							<MainBtn
								...
								style={{
									backgroundColor:
										user.invite_status == 'invited' ? Theme.colors.gray7 : Theme.colors.cyan2,
								}}
								title={
									user.invite_status == 'invited'
										? translate('friend_related.cancel_invitation')
										: translate('friend_related.add_friend')
								}
								onPress={
									user.invite_status == 'invited' ? this.onCancelInvitation : this.onSendInvitation
								}
							/>
						)}
					</View>
					...
					FooterComponent={({ imageIndex }) => {
						return (
							<Text style={styles.indicator}>
								{imageIndex + 1} / {this.state.galleryImages.length}
							</Text>
						);
					}}
				/>
...
export default connect(mapStateToProps, { goActiveScreenFromPush })(withNavigation(SnapfooderScreen));
