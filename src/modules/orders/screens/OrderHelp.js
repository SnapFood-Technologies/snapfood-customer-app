...
import { order_support_collection, getOrderSupportChannel, createOrderSupportChannel } from '../../../common/services/order_support';
...
const OrderHelp = (props) => {
    ...
    const orderHelpBlockedDescription = useMemo(() => {
        ...
        order.created_at, props.systemSettings.order_help_block_days, 
    ...
    const onSupport = async () => {
        ...
        if (channelData) {
            ...
            let channelId = await createOrderSupportChannel(order, user, props.language);
            isChannelCreating.current = false;
            if (channelId) {
                ...
                >
                    ...
                    <View style={styles.divider} />
                    {
                        btns.map((btn, index) =>
                            ...
                            >
                                <AppText style={[styles.itemTxt, Theme.styles.flex_1]}>{translate('help.' + btn.name)}</AppText>
                            </TouchableOpacity>
                        )
                    }
                    {
...
const styles = StyleSheet.create({
    title: { marginVertical: 20, width: '100%', paddingHorizontal: 20, textAlign: 'center', fontSize: 19, lineHeight: 20, fontFamily: Theme.fonts.bold, color: Theme.colors.text },
