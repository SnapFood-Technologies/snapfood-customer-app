...
const SplitHistItem = ({ data, onPress, style }) => {
    ...
    return <TouchableOpacity onPress={() => onPress()}
        ...
        <View style={[Theme.styles.flex_between]}>
            <AppText style={[styles.name]}><Text style={{ fontFamily: Theme.fonts.semiBold }}>{translate('splits_hist.orderer')}:</Text> {data?.order_data?.customer_full_name}</AppText>
            <AppText style={[styles.name, { fontFamily: Theme.fonts.semiBold }]}>{data?.order_data?.vendor_title} </AppText>
        </View>
        <View style={[Theme.styles.flex_between, { marginTop: 12 }]}>
            <AppText style={[styles.priceTxt]}><Text style={{ fontFamily: Theme.fonts.semiBold }}>{translate('splits_hist.total_amount')}:</Text> {data?.order_data?.total_price} L</AppText>
            <View />
        </View>
        <View style={[Theme.styles.flex_between, { marginTop: 12 }]}>
