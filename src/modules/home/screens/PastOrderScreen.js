...
const PastOrderScreen = (props) => {
	...
	const onDeleteOrder = (order_id) => {
		alerts.confirmation(translate('order.confirm_delete'), '').then(() => {
			changeOrderStatus(order_id, 'deleted')
				.then((res) => {
					loadPastOrders(vendorInfo.vendor_id);
				})
				.catch((err) => {});
	...
	const onCancelOrder = (order_id) => {
		alerts.confirmation(translate('order.confirm_cancel'), '').then(() => {
			changeOrderStatus(order_id, 'canceled')
				.then((res) => {
					loadPastOrders(vendorInfo.vendor_id);
				})
				.catch((err) => {});
...
const styles = StyleSheet.create({
	...
	brand_name: { marginLeft: 8, fontSize: 19, fontFamily: Theme.fonts.bold, color: Theme.colors.text },
...
export default connect(mapStateToProps, {})(PastOrderScreen);
