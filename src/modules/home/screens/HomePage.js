...
import { AppState, Image, Platform, ActivityIndicator, ScrollView, TouchableOpacity, Text, View, StyleSheet, RefreshControl, Keyboard, KeyboardAvoidingView } from 'react-native';
...
import {
    setHomeVendorFilter, setHomeVendorSort, loadInvitationTimerSetting, getReferralsRewardsSetting,
    getActiveVendors, setShowWhereHeardFeedbackModal, getSystemSettings, setShowAnnounceModal,
    setInvitationTimerSetting, setLocationDiffTooltip, setHomeScroller, getPromotionBanner, getAllPromotionBanners,
...
const HomePage = (props) => {
    ...
    useEffect(() => {
        ...
        if (_appStateListener.current) {
            console.log("=========== new app state listener");
            try {
        ...
        _appStateListener.current = AppState.addEventListener("change", nextAppState => {
            if (
                ...
                console.log('========== home loadInvitationTimerSetting');
                props.loadInvitationTimerSetting();
            }

            appState.current = nextAppState;
            if (appState.current == 'background') {
                console.log("========== App has gone to the background!,  hide invite timer");
    ...
    const getFilers = (flag) => {
        ...
        const { vendor_type, order_type, is_meal, is_dietary, ongoing_offer, open_now, online_payment, delivery_by_snapfood, low_fee, high_fee, searchTerm } = props.home_vendor_filter;
        let filters = []
    ...
    const _renderCategories = () => {
        ...
        return <View style={[Theme.styles.col_center, { marginTop: 8 }]}>
            <View style={[Theme.styles.row_center, styles.categoryWrap]}>
                {
                    foodCategories.slice(0, (currentCategoryRow == 1 ? 4 : (currentCategoryRow == categoryRowLength ? foodCategories.length : currentCategoryRow * 4))).map((cat, index) =>
                        <View key={cat.id}
                            style={[Theme.styles.col_center, styles.categoryItemWrap]}
                        >
                            <CategItem
                                category={cat}
                                cat_id={cat.id}
    ...
    const _renderFeatureBlocks = () => {
        const validBlocks = expectedBlocks.filter(({ key, icon }) => {
            return (featuredBlocks[key] != null && featuredBlocks[key].block && featuredBlocks[key].block['is_active'] &&
                featuredBlocks[key].vendors && featuredBlocks[key].vendors.length > 0 && featuredBlocks[key].block.display_count > 0)
        })
        ...
        return <View style={{ width: '100%', marginTop: 8, }}>
            {
                ...
                (
                    <>
                        <FeatureList
                            label={props.language == 'sq' ? featuredBlocks[validBlocks[0].key].block.title_sq : featuredBlocks[validBlocks[0].key].block.title_en}
                            items={
                                validBlocks[0].key == 'order_again' ?
                                    featuredBlocks[validBlocks[0].key].vendors.slice(0, featuredBlocks[validBlocks[0].key].block.display_count)
                                    :
        ...
        return (
            <View style={[Theme.styles.row_center, { width: '100%', height: 50, marginBottom: 6, }]}>
                <AutoCompleteInput
                    ...
                    fontSize={17}
                    isSearch={true}
                    data={(suggested.length == 1 && suggested[0].keyword.toLowerCase() == searchtext.toLowerCase()) ? [] : suggested}
                    value={searchtext}
                ...
                />
                <RoundIconBtn style={{ position: 'absolute', top: 0, right: 0, width: 49, height: 49 }} icon={<MaterialIcons name='filter-list' size={26} color={Theme.colors.cyan2} />}
                    onPress={() => goRootStackScreen(RouteNames.FilterScreen)} />
            </View>
        );
    };
    ...
    const _renderVertVendors = () => {
        ...
        return <View style={[Theme.styles.col_center_start, { width: '100%', alignItems: 'flex-start', minHeight: height(40) }]}>
            ...
            >
                {
                    (banner_pos_in_vendors < 0 ? allvendors : allvendors.slice(0, banner_pos_in_vendors))
                        .map((vendor, index) =>
                ...
                {
                    banner_pos_in_vendors >= 0 &&
                    (allvendors.slice(banner_pos_in_vendors))
                        .map((vendor, index) =>
                            <View key={vendor.id} style={[Theme.styles.col_center, { width: '100%', }]}>
                                <VendorItem
                                    data={vendor}
    ...
    return (
        <View style={[Theme.styles.col_center_start, { flex: 1, backgroundColor: Theme.colors.white }]}>
            ...
            >
                ...
                <View style={[Theme.styles.row_center, styles.operationTab]}>
                    <SwitchTab
                        ...
                        style={{ width: '100%' }}
                        activetxt_style={props.language == 'en' ? { fontSize: 19, lineHeight: 20 } : { fontSize: 19, lineHeight: 20 }}
                        inactivetxt_style={props.language == 'en' ? { fontSize: 19, lineHeight: 20 } : { fontSize: 19, lineHeight: 20 }}
                    />
                </View>
                {
                    dataLoading != false ?
                        <View style={{ height: (height(100) - (props.isLoggedIn ? 300 : 260)), width: '100%', backgroundColor: Theme.colors.white }}>
                            <BlockSpinner />
                        </View>
                        :
                        <>
                                ...
                                (isEmptyData() == true) ?
                                    ...
                                    <>
                                        ...
                                        <View style={{ width: '100%' }}>
                                            ...
                                            <View style={{ position: 'absolute', left: 0, top: 0, width: width(100) - 40, flexDirection: 'row', alignItems: 'center', }}>
            ...
            {
                props.referralsRewardsSetting.show_earn_invitation_module == true &&
                props.invitationTimerSetting.showTimer == true &&
                <InviteTimer style={styles.timer} />
            }
...
const styles = StyleSheet.create({
    ...
    leftArrowView: { position: 'absolute', left: 0, top: 0, zIndex: 10, borderTopRightRadius: 15, borderBottomRightRadius: 15, height: '100%', paddingHorizontal: 4 },
    rightArrowView: { position: 'absolute', right: 0, top: 0, borderTopRightRadius: 15, borderBottomRightRadius: 15, height: '100%', paddingHorizontal: 4 },
...
export default connect(mapStateToProps, {
    setHomeVendorFilter, setHomeVendorSort, setVendorCart, getFeaturedBlocks,
    toggleFavourite, loadInvitationTimerSetting, getReferralsRewardsSetting,
    getActiveVendors, setShowWhereHeardFeedbackModal, getSystemSettings,
    setShowAnnounceModal, setInvitationTimerSetting, setLocationDiffTooltip,
    setHomeScroller, getPromotionBanner, getMembershipSetting, getAllPromotionBanners
})(HomePage);