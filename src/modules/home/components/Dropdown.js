...
const Dropdown = ({ list_items, value, style,defaultValue, item_height, onChange, textStyle }) => {
    ...
    return (
		<ModalDropdown
			...
			options={list_items}
			defaultValue={defaultValue || value || list_items[0]}
			...
			dropdownStyle={[
				...
				style,
				{
					...
					height: list_items.length * item_height + 10,
				},
			]}
			renderRow={(option, index, isSelected) => (
				<View
					style={[
						...
						{ backgroundColor: '#fff', height: item_height, justifyContent: 'flex-start' },
					]}
				>
					<Text style={[styles.text, !isSelected && { color: Theme.colors.gray7 }, textStyle]}>
						{translate(list_items[index])}
					</Text>
				</View>
			)}
			renderSeparator={(index) => <View style={styles.divider} />}
		>
			<View style={[Theme.styles.row_center, styles.container, style]}>
				<Text style={[styles.text, textStyle]}>{translate(value)}</Text>
