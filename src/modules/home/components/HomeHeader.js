...
const MIN_W = 386
const HomeHeader = ({ language, curTab, coordinates, isLoggedIn, cashback_amount, photo, locationTooltipMsg, onClearLocationTooltip, onLocationSetup, onGoWallet, onGoProfile, onTabChange }) => {

    console.log('--- HomeHeader ---')
    return <View style={[Theme.styles.row_center, styles.header, { marginTop: (isLoggedIn ? 2 : 43) }]}>
        <Switch
        ...
        <View style={[Theme.styles.row_center_end, { flex: 1, alignItems: 'flex-end' }]}>
            <Tooltip
                ...
                content={
                    <View style={styles.tooltip}>
                        <AppText style={{ fontSize: 14, lineHeight: 22, fontFamily: Theme.fonts.semiBold, color: '#FFF' }}>
                            {locationTooltipMsg}
                        </AppText>
                    </View>
                }
                ...
                disableShadow={false}
                onClose={() => { onClearLocationTooltip() }}
            >
                <RoundIconBtn style={{ width: 42, height: 42 }} icon={<Entypo name='location-pin' size={26} color={Theme.colors.cyan2} />}
            ...
            </Tooltip>
            {
                ...
                <View style={{ width: 48, height: 48, marginLeft: 10, justifyContent: 'flex-end' }}>
                    <RoundIconBtn style={{ width: 42, height: 42 }}
