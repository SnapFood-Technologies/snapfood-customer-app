...
const MembershipFaqs = (props) => {
    ...
    const loadFaqs = () => {
        apiFactory.get(`get-membership-faqs`)
            .then(({ data }) => {
                let faqs = data.faqs || [];
    ...
    const increaseFaqCount = (faq_id) => {
        apiFactory.post(`membership/increase-clicks-faqs`, {
            tag_id: faq_id
        })
            .then((res) => { }, (error) => { });
    }
    ...
    return (
        <React.Fragment>
            <HelpPage title={translate('membership.faq_title')} titleStyle={{width: 200}} bodystyle={{ marginTop: 70 }} navigation={props.navigation}>
