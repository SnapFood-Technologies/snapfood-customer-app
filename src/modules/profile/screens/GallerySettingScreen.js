...
const GallerySettingScreen = (props) => {
	...
	return (
		<View style={styles.container}>
			<Header1
				...
				title={translate('gallery.privacy')}
			/>
					...
					<View style={[Theme.styles.col_center, styles.sectionView]}>
						<View style={[Theme.styles.col_center_start, styles.language]}>
							<View style={[Theme.styles.row_center, { width: '100%', marginBottom: 16 }]}>
								<Text style={[Theme.styles.flex_1, styles.subjectTitle]}>{translate('gallery.privacy')}</Text>
							</View>
							<TouchableOpacity
								onPress={() => onUpdateNoti({ gallery_public: props.user.gallery_public == 1 ? 0 : 1 })}
								style={[Theme.styles.row_center, { width: '100%', marginBottom: 16, }]}
							>
								<View>
									<Text style={[styles.langtxt]}>{translate('gallery.public')}</Text>
									<Text style={[styles.setting_desc]}>{translate('gallery.public_desc')}</Text>
								</View>
								<View style={{ flex: 1 }} />
								<RadioBtn
									onPress={() => onUpdateNoti({ gallery_public: props.user.gallery_public == 1 ? 0 : 1 })}
									checked={props.user.gallery_public == 1}
								/>
							</TouchableOpacity>
							...
							>
								<View>
									<Text style={[styles.langtxt]}>{translate('gallery.private')}</Text>
									<Text style={[styles.setting_desc]}>{translate('gallery.private_desc')}</Text>
								</View>
								<View style={{ flex: 1 }} />
								<RadioBtn
									onPress={() => onUpdateNoti({ gallery_public: props.user.gallery_public == 1 ? 0 : 1 })}
									checked={props.user.gallery_public == 0}
								/>
							</TouchableOpacity>
						</View>
					</View>
