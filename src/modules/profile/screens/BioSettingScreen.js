	...
	return (
		<View style={styles.container}>
			<Header1
				...
				title={translate('biosetting.privacy')}
			/>
					...
					<View style={[Theme.styles.col_center, styles.sectionView]}>
						<View style={[Theme.styles.col_center_start, styles.language]}>
							<View style={[Theme.styles.row_center, { width: '100%', marginBottom: 16 }]}>
								<Text style={[Theme.styles.flex_1, styles.subjectTitle]}>{translate('biosetting.privacy')}</Text>
							</View>
							<TouchableOpacity
								onPress={() => onUpdateNoti({ bio_text_public: props.user.bio_text_public == 1 ? 0 : 1 })}
								style={[Theme.styles.row_center, { width: '100%', marginBottom: 16, }]}
							>
								<View>
									<Text style={[styles.langtxt]}>{translate('biosetting.public')}</Text>
									<Text style={[styles.setting_desc]}>{translate('biosetting.public_desc')}</Text>
								</View>
								<View style={{ flex: 1 }} />
								<RadioBtn
									onPress={() => onUpdateNoti({ bio_text_public: props.user.bio_text_public == 1 ? 0 : 1 })}
									checked={props.user.bio_text_public == 1}
								/>
							</TouchableOpacity>
							...
							>
								<View>
									<Text style={[styles.langtxt]}>{translate('biosetting.private')}</Text>
									<Text style={[styles.setting_desc]}>{translate('biosetting.private_desc')}</Text>
								</View>
								<View style={{ flex: 1 }} />
								<RadioBtn
									onPress={() => onUpdateNoti({ bio_text_public: props.user.bio_text_public == 1 ? 0 : 1 })}
									checked={props.user.bio_text_public == 0}
								/>
							</TouchableOpacity>
						</View>
					</View>
