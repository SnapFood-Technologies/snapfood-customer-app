...
export const setShowContactsModal = (payload) => (dispatch, getState) => {
    return new Promise(async resolve => {
        if (payload == true) {
            ...
            console.log('hide_contacts_modal ', hide_contacts_modal)
...
export const getAllMyFriends = () => dispatch => {
    return new Promise(resolve => {
        apiFactory.post('users/friends', { status: 'accepted' })
            .then(({ data }) => {
                dispatch({
...
export const getReferralsRewardsSetting = () => async (dispatch, getState) => {
    return new Promise(resolve => {
        apiFactory.get(`invite-earn/get-referrals-setting`)
            .then(async ({ data }) => {
                await dispatch({
                    type: APP.SET_REFERRALS_REWARDS,
                    payload: {
...
export const getMembershipSetting = () => async (dispatch, getState) => {
    return new Promise(resolve => {
        apiFactory.get(`membership-setting`)
            .then(async ({ data }) => {
                await dispatch({
...
export const saveInterestsFromOnboard = () => {
    return new Promise(async (resolve) => {
        ...
        if (interests.length > 0) {
            apiFactory.post(`interests`, { interests: interests.map(i => i.id) })
                .then(async ({ data }) => {
                    try {
...
export const loadInvitationTimerSetting = () => async (dispatch, getState) => {
    return new Promise(resolve => {
        apiFactory.post(`/invite-earn/check-earninvitation`)
            .then(async ({ data }) => {
                let showTimer = false;
...
export const checkLocationDiff = (location, old_location) => dispatch => {
    ...
    `old_lat=${old_location.latitude}`, `old_lng=${old_location.longitude}`];

    return new Promise(resolve => {
        apiFactory.get(`check-location-diff?${params.join('&')}`).then(({ data }) => {
            console.log(data)
            dispatch({
...
export const getActiveVendors = (latitude, longitude) => dispatch => {
    ...
    return new Promise(resolve => {
        apiFactory.get(`vendors/active?${params.join('&')}`).then(({ data }) => {
            // console.log('============ active vendors ', data.vendors?.length, data.orders, data.has_splits)
            dispatch({
...
export const setShowAnnounceModal = (payload) => (dispatch, getState) => {
    return new Promise(async resolve => {
        if (payload == true) {
            ...
            if (announceData != null && announceData.id != null && (prev_announce_id == null || announceData.id > prev_announce_id)) {
...
export const getSystemSettings = () => dispatch => {
    return new Promise(resolve => {
        apiFactory.get(`get-settings`).then(({ data }) => {
            dispatch({
...
export const getStudentVerifySettings = () => dispatch => {
    return new Promise(resolve => {
        apiFactory.get(`promotions/student-verification-setting`).then(({ data }) => {
            dispatch({
...
export const getPromotionBanner = (promotion_type = 'vendor') => dispatch => {
    return new Promise(resolve => {
        apiFactory.get(`banners/promotion?promotion_type=${promotion_type}`).then(({ data }) => {
            dispatch({
...
export const getAllPromotionBanners = () => dispatch => {
    return new Promise(resolve => {
        apiFactory.get(`banners/all-promotions`).then(({ data }) => {
            dispatch({
                ...
                if (cnt >= show_interval) {
                    try {
                        ...
                        if (res.data && res.data.snapfooders && res.data.snapfooders.length > 0) {
                            await dispatch({ type: APP.SET_SHOW_INVITE_MUTUAL_FRIEND_MODAL, show: true, data: res.data.snapfooders });
                        }
                    } catch (error) {
                        console.log(error);
                    }
