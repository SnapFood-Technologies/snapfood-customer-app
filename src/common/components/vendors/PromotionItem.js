...
const PromotionItem = ({ data, language, isPast = false, onSelect, onLongPress = () => { }, style }) => {
    ...
    return <TouchableOpacity onPress={() => onSelect()}
        ...
        <View style={[Theme.styles.row_center_start, { width: '100%', marginBottom: 4, }]}>
            <Text style={[styles.title, { flex: 1, }]}  >{(language == 'en' && !isEmpty(data.title_en)) ? data.title_en : data.title}</Text>
            <View style={{ alignItems: 'flex-end' }}>
                {
                    (isPast != true && data.non_expired != 1) &&
                    (
                        getDateLimit() > 0 ?
                            <Text style={styles.date_limit}>{getDateLimit()} {translate('restaurant_details.days_left')}</Text>
                            :
        ...
        <View style={[Theme.styles.row_center, Theme.styles.w100, { marginTop: 4 }]}>
            <View style={[Theme.styles.col_center_start, { flex: 1, alignItems: 'flex-start' }]}>
                {
                    data.is_vendor_promotion == 1 && data.promotion_vendor && <Text style={[styles.sub_title]}>{data.promotion_vendor.title}</Text>
                }
...
const styles = StyleSheet.create({
    ...
    descTxt: { marginTop: 4, marginBottom: 8, fontSize: 16, lineHeight: 19, color: Theme.colors.gray7, fontFamily: Theme.fonts.medium, },
