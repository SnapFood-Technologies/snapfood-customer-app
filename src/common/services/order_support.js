...
export const createOrderSupportChannel = async (order, user, language, systemSettings) => {
	try {
		let members = [
			SNAP_FOOD_ADMIN_CONTACT,
			{
		...
		await sendMessage(channelId, SNAP_FOOD_ADMIN_CONTACT.id, {
			text:
				language == 'en'
					? 'Write every question or request you might have regarding your order!'
					: 'Shkruaj çdo pyetje apo kërkesë për porosinë tënde!',
			system: true,
		});
		...
		let operatorName = '';
		if (order.operator_name_en && order.operator_name_it && order.operator_name_sq) {
			if (language == 'en') {
				operatorName = order.operator_name_en;
			} else if (language == 'it') {
				operatorName = order.operator_name_it;
			} else {
				operatorName = order.operator_name_sq;
			}
		} else if (
			systemSettings &&
			systemSettings.operator_name_en &&
			systemSettings.operator_name_it &&
			systemSettings.operator_name_sq
		) {
			if (language == 'en') {
				operatorName = systemSettings.operator_name_en;
			} else if (language == 'it') {
				operatorName = systemSettings.operator_name_it;
			} else {
				operatorName = systemSettings.operator_name_sq;
			}
		} else {
			operatorName = 'SnapFood Support';
		}

		await sendMessage(channelId, SNAP_FOOD_ADMIN_CONTACT.id, {
			text:
				language == 'en'
					? `Hello ${firstName}, I am ${operatorName} from Snapfood and I’m here to help you with your order.`
					: `Përshëndetje ${firstName}, jam ${operatorName} nga Snapfood për të të ndihmuar me gjithçka lidhur me porosinë.`,
			user: {
				...SNAP_FOOD_ADMIN_CONTACT,
...
export const setLike = async (channelData, user_id, msg, onSuccess = () => {}) => {
	try {
		if (channelData != null && channelData.users && msg != null) {
			...
			if (foundIndex == -1) {
				// when new add
				if (msg.user && msg.user._id != user_id) {
					// if this messge is not my message
...
export const sendMessage = async (channelId, user_id, message) => {
	try {
		...
		await order_support_collection.doc(channelId).collection('messages').doc(message._id).set(new_msg);
...
export const deleteMessage = async (channelId, messageId) => {
	try {
		await order_support_collection.doc(channelId).collection('messages').doc(messageId).delete();
	} catch (err) {}
};
...
export const sendGroupChatInviteNotification = (conversation_id, group_name, member_ids) => {
	apiFactory
		.post('chats/send-groupchat-invite', {
...
export const sendChatNotification = (
	...
	apiFactory
		.post('chats/send-chat-notification', {
			conversation_id: conversation_id,
...
export const updateChannelUserInfo = async (user) => {
	try {
		let channel_creator_ref = await order_support_collection
			.where('channel_type', '==', 'single')
			.where('creator.id', '==', user.id)
			.get();

		let channel_partner_ref = await order_support_collection
			.where('channel_type', '==', 'single')
...
export const exitGroupChannel = async (channelData, user_id) => {
	try {
		...
		if (channelData.admin != null && channelData.admin.id == user_id) {
			await order_support_collection
				.doc(channelData.id)
				.update('admin', new_members[0], 'members', new_members, 'users', new_users);
		} else {
			await order_support_collection.doc(channelData.id).update('members', new_members, 'users', new_users);
		}
		return true;
	} catch (error) {
		return false;
